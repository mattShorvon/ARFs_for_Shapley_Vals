...
# version that doesn't rank by abs(phi) for easier comparison
plot.shapr <- function(x,
                       digits = 3,
                       plot_phi0 = TRUE,
                       index_x_test = NULL,
  ...
  plotting_dt <- plotting_dt[id %in% index_x_test]
  plotting_dt[, rank := c(5,4,3,2,1), by = "id"]
  plotting_dt <- plotting_dt[rank <= top_k_features]
  plotting_dt[, description := factor(description, levels = unique(description[order(rank)]))]
