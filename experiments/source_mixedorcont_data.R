...
compute_shapley_mixedorcont_data <- function(parameters_list){
  ...
  mu <- rep(0, No_tot_var)
  beta_0 <- 1
  if (No_cat_var == 0) { # recreating Aas et al. continuous results
    beta_cont <- rep(1, No_cont_var)
  } else {
    beta_cont <- c(1, -1)
  }
  # beta_cat <- c(1, 0, -1,
  ...
  beta_cat <- c(1, 0, -1, 0.5,
                2, 3, -1, -0.5)
  if (No_cat_var == 0) {
    beta <- c(beta_0, beta_cont)
  } else {
    beta <- c(beta_0, beta_cont, beta_cat) # change for only cont
  }
  ...
  if (No_cat_var == 0) {
    names(dt_train) <- c(paste0("cont_", 1:No_cont_var, "_"))
  } else {
    names(dt_train) <- c(paste0("cont_", 1:No_cont_var, "_"), paste0("cat_", 1:No_cat_var, "_"))
  }

  if(noise == TRUE){
    dt_train[, epsilon := rnorm(No_train_obs, 0, 0.01^2)] # using same noise as Aas et al.
  } else{
    dt_train[, epsilon := 0]
  }
  ...
  if (No_cat_var == 0) {
    names(dt_test) <- c(paste0("cont_", 1:No_cont_var, "_"))
  } else {
    names(dt_test) <- c(paste0("cont_", 1:No_cont_var, "_"), paste0("cat_", 1:No_cat_var, "_"))
  }

  if(noise == TRUE){
    dt_test[, epsilon := rnorm(No_test_obs, 0, 0.01^2)] # using Aas et al. noise term
  } else{
    dt_test[, epsilon := 0]
  }
  ...
  # hopefully no change for only cont
  if (No_cat_var == 0) {
    mod_matrix <- model.matrix(~.-1,dt[, ..cont_cols])
  } else {
  ...
  predict_model.numeric_lm <<- function(x, newdata) {
    ...
    if (No_cat_var != 0) {
      cat_cols <- names(newdata)[grep("cat",names(newdata))] # change for only cont maybe? could add an if to skip this
  ...
  } # This case_matrix thing doesn't seem to be used for anything
  ...
  # these are the values used for the variables being integrated over in the numerical integration.
  ...
  # basically just combines all the beta terms into one list object, where the list indices
  # correspond to the feature number that the beta parameter corresponds to.
  ...
  Vs_mat <- matrix(NA, ncol = No_test_obs, nrow = nrow(S)) # this is the matrix of subset values
  Vs_mat[1, ] <-   phi0
  Vs_mat[nrow(S),] <- predict(model, x_test)
  # this is the matrix of subset values, rows are different subsets, columns are different test instances
  # first value is phi0, last value is the prediction for the full coalition.

  # algorithm <- mvtnorm::GenzBretz() # Not exact and slower for small dimensions
  algorithm <- mvtnorm::Miwa()

  start <- proc.time()
  mc.cores <- 1 # change this to speed up computation
  for (i in 2:(nrow(S)-1)){
    # i <- 2
    S_i <-   which(as.logical(S[i,]))         # feature numbers that are in/out of coalition
    Sbar_i <-   which(as.logical(1-S[i,]))

    S_features <- feat_names[S_i]

    x_test_S_i <- x_test[,..S_features] # array of values for the in-coalition features

    x_test_C_lower_S_i_list <- split(x_test_C_lower[, ..S_features],f = 1:No_test_obs) # Lower/Upper values for in-coalition features, stored as a list for some reason
    ...
    for (j in Sbar_i){
      ...
      prep_list_1 <- mapply(prep_dens_x_given_S_is_C_func,
                            ...
                            SIMPLIFY = FALSE)
      # stores constant parameters needed for eqn 13, for each test point, in one big list.

      prep_list_2[[j]] <- unlist(lapply(prep_list_1, prep_dens_x_given_S_is_C_func_v2, x_vec = x_int_grid), recursive = F)
    }
    # this gets the parameters that depend on the value x being integrated over. This is why x_int_grid is sent as a param,
    # it contains these values being used in the numerical integration (there are 500 of them for this specific case).
    # We end up with 25000 values for each feature not in the coalition that is integrated over, because each feature has
    # 500 test values and the numerical integration is being carried out with 500 values in this case.

    # this gets you the conditional densities for each of the 500 elements of the integrals for each of the 500 x_test vals,
    # for each of the out-of-coalition features that an expectation needs to be calced for (in this case there's 3 of them).
    ...
    # then it uses these to find the expectations.
    jj <- 1
    for (j in Sbar_i){
    ...
    Vs_mat[i,] <- rowSums(Vs_sum_contrib_mat) # find out what x_test_onehot_full_list cont cols look like to rewrite this part
    print(i)
  }
  end <- proc.time()
  ...
  if (No_cat_var != 0) {
    ...
    phi_sum_mat <- matrix(NA, nrow = No_test_obs, ncol = No_cont_var)
  }
