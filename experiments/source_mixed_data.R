...
compute_shapley_mixed_data <- function(parameters_list){
  ...
  # createToeplitzCovarianceMatrix <- function(size, rho) {
  #   indices <- 1:size
  #   cov_matrix <- outer(indices, indices, function(i, j) rho^abs(i - j))
  #   return(cov_matrix)
  # }
  # Sigma <- createToeplitzCovarianceMatrix(No_tot_var, corr)
  # diag(Sigma) <- Sigma_diag
  # print(Sigma)
