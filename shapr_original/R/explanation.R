...
explain <- function(x, explainer, approach, prediction_zero,
  ...
  if (!(is.vector(approach) &&
    ...
    all(is.element(approach, c("empirical", "gaussian", "copula", "ctree", "independence", "arf","arf_sampling"))))
  ) {
    stop(
      paste(
...
prepare_and_predict <- function(explainer, n_batches, prediction_zero, ...) {
  ...
  for (batch in seq_along(S_batch)) {
    ...
    # View(dt, "dt just after prepare_data, line 679") #is correct
    r_batch[[batch]] <- prediction(dt, prediction_zero, explainer)
    r_batch[[batch]]$dt_mat[, row_id := S_batch[[batch]]]
    # View(r_batch[[1]]$dt_mat, "r_batch dtmat line 982") # same as dt_mat later on
    # View(r_batch[[1]]$p, "r_batch p line 982") # just 53.9 for some reason
