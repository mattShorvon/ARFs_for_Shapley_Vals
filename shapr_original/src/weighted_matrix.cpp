...
arma::mat weight_matrix_cpp(List subsets, int m, int n, NumericVector w){
    ...
    // where X.t() = Z.t() * W. This is equation 7! R is multiplied by v to find the 
    // shapley values vector phi.  
    ...
    R = inv(X.t() * Z) * X.t(); // this corresponds exactly with how R is stated as being calculated in Aas et al. eq 6
