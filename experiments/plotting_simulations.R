...
# getting error bars:
rand_string_list <- c("AWGHR","BDRYB","ltsly","WI1Ao")
results_1 <- list()
results_2 <- list()
results_3 <- list()
results_4 <- list()
...
for (i in 1:length(rand_string_list)) {
  ...
  no_categories <- 8
  print(rand_string)
  folder <- paste0(tod_date, "_", rand_string, "_dim", dim, "nbcat2cont2_toeplitz_", no_categories)
  nm <- paste0(tod_date, "_", rand_string, "_dim", dim, "nbcat2cont2_toeplitz_", no_categories, "_rho_0.9.rds")
  all_methods <- readRDS(paste("C:/Users/mshor/OneDrive - King's College London/PhD/ShapleyValExptData/dim4_nbcat2cont2_toeplitz", folder, nm, sep = "/"))
  MAE_truth <- NULL
  MAE_methods <- NULL
  MAE_methods_names <- NULL
  MAE_parameters <- NULL
